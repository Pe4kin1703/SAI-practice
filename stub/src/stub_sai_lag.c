...
{
    printf("%s: Called for lag_id: 0x%lx\n", __FUNCTION__, lag_id);
    return SAI_STATUS_NOT_IMPLEMENTED;
}
    ...
    _In_ sai_attribute_t *attr_list)
{
    ...
    sai_uint32_t lag_id = ((stub_object_id_t*)&attr_list[0].value.oid)->data;
    printf("CREATE LAG MEMBER: 0x%lX {LAG_ID: %u, PORT_ID: %ld}\n", *lag_member_id, lag_id, attr_list[1].value.oid);
    ;
    return SAI_STATUS_SUCCESS;
}
...
sai_status_t stub_get_lag_member_attribute(
    ...
    _Inout_ sai_attribute_t *attr_list)
{
    printf("%s: Called for lag_member_id: 0x%lx\n", __FUNCTION__, lag_member_id);
    return SAI_STATUS_NOT_IMPLEMENTED;
}
